...
SAD=zeros(dispmax,size(left_res,2)-4);
depth=zeros(size(left_in,1),size(left_in,2)+2);
SAD_rc=zeros(1,dispmax);
reverseStr = '';
Crosscheck=2;
...
for y=wd2+1:1:size(left_res,1)-wd2 %Left image row pix
    ...
    for x=wd2+1:1:size(left_res,2)-wd2  %Left image col pix
        ...
        for x_disp=dispmin:1:dispmax
           if x-wd2-x_disp<=wd2 %ignore black region on the left, right bound is constrainted by x
               break
           end
              right=right_res(y-wd2:y+wd2,x-wd2-x_disp:x+wd2-x_disp);
              SAD(x_disp,x)=sum(abs(left(:)-right(:))); %take the sum of absolute difference
              %SSD(disp)=SAD.^2;
              max_move_left=x_disp;
           
        end
        SAD(SAD(:,x)>0,x)=SAD(SAD(:,x)>0,x)/max(SAD(:,x));
    end
        w     = 5;       % bilateral filter half-width
        sigma = [2 0.1]; % bilateral filter standard deviations

        %5 3 0.1
        bflt_SAD = bfilter2(SAD,w,sigma);
        bflt_SAD (bflt_SAD<0.001)=0;
        [temp,depth(y,:)]=max(bflt_SAD);
...
%                    depth(y-wd2,x-wd2)=0;%min(depthL,depth_rc);
