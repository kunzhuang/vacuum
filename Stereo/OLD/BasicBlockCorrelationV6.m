...
for y=wd2+1:1:size(left_res,1)-wd2 %Left image row pix
    ...
    for x=wd2+1:1:size(left_res,2)-wd2  %Left image col pix
        ...
        for disp=dispmin:1:dispmax
           if x-wd2-disp<wd2 %ignore black region
               break
           end
        ...
        if dispmin>=disp-1
            depth(y-wd2,x-wd2)=0; %ignore black region and assign range to infinity
        else
            [temp,depth(y-wd2,x-wd2)]=min(SAD(dispmin:disp-1));
        end
