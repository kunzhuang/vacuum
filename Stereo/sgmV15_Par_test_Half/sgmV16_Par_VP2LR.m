...
PixelCost=CalculatePixelCost(right_in,left_in,disparityRange,'LR');
...
parfor p=1:PathNum
AGCostSUM=AGCostSUM+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,right_in,1);
end
