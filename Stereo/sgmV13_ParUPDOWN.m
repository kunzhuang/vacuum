...
left_in=imresize(left_in,0.5);
right_in=imresize(right_in,0.5);
...
AGCost_Par=zeros(Rows,Cols,disparityRangeLevels,16);
...
parfor p=1:PathNum
AGCost_Par(:,:,:,p)=PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost);
end
for p=1:PathNum
    [~,PP.(sprintf('Path_%i', p))]=min(AGCost_Par(:,:,:,p),[],3);
    save('PathFile.mat','PP','-append')
    AGCostSUM=AGCostSUM+AGCost_Par(:,:,:,p);
end
...
figure
disparityMapSUM=uint8(imresize(disparityMapSUM,2));
...
xyzPoints = reconstructScene(disparityMapSUM.disparityMapSUM,CamCalib);
    ...
    for Col=1:Cols
        ...
        for Row=1:Rows
            ...
            if (LrLastRow <= 0 | LrLastRow >= Rows | LrLastCol<= 0| LrLastCol>= Cols) %check borders
                ...
                LrLastMin=min(AGCost(LrLastRow,LrLastCol,:)); %Calculate global min on LrLast->Lr(p-r,k), min cost value, not disparity
                AGCost(Row,Col,:)=PixelCost(Row,Col,:);
