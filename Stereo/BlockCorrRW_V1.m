...
DisparitySpaceImageRightInLeftCoord=NaN(size(left_in,1),size(left_in,2),dispmax-dispmin+1);
...
%     %Padding zeros to right image
%     else
%         left_d_window(:,:,d_window)=zeros(size(right_in_conv2,1),size(right_in_conv2,2));
%     end
% end
...
for d_LeftToRightCoord=1:dispmax-dispmin+1
    d_window_val=dispmin:dispmax;
    if d_window_val(d_LeftToRightCoord)<size(right_in_conv2,2)
    DisparitySpaceImageRightInLeftCoord(:,:,d_LeftToRightCoord)=[zeros(size(right_in_conv2,1),d_window_val(d_LeftToRightCoord)),DisparitySpaceImageRight(:,1:end-d_window_val(d_LeftToRightCoord),d_LeftToRightCoord)];
    end
end

DisparitySpaceLeft=medfilt2(ComputeDisparity(Rows, Cols, disparityRange,DisparitySpaceImageLeft));
DisparitySpaceRight=medfilt2(ComputeDisparity(Rows, Cols, disparityRange,DisparitySpaceImageRight));
DisparitySpaceRightInLeftCoord=medfilt2(ComputeDisparityNaN(Rows, Cols, disparityRange,DisparitySpaceImageRightInLeftCoord));

if Crosscheck
    UniqeCheck=zeros(Rows,Cols);
    UniqeCheck(abs(DisparitySpaceLeft-DisparitySpaceRightInLeftCoord)>disp_th)=1;
    DisparitySpace=DisparitySpaceLeft;
    DisparitySpace(logical(UniqeCheck))=0;
else
    DisparitySpace=DisparitySpaceLeft;
end
...
figure
imshow(255*DisparitySpaceRightInLeftCoord/max(max(DisparitySpaceRightInLeftCoord)),[1,255])
colormap jet
colorbar
