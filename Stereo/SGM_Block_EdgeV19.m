...
AGCostSUMR=single(zeros(Rows,Cols,disparityRangeLevels));
 ...
 if SGM
     ...
     parfor p=1:PathNum %Left image
         AGCostSUML=AGCostSUML+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCostL,left_in,left_rectf_crop_Canny,0,1);
     end
     
     parfor p=1:PathNum %Right image
         AGCostSUMR=AGCostSUMR+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCostR,right_in,right_rectf_crop_Canny,0,2);
     end
 else
...
[rms,ssim]=RMSErr(disparityMapSUMLM.*LRRL,LRRL);
rms
ssim
...
function AGCost=PathScan(p,RR,CC,Corn,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,Img,EdgeCoord,Save,s)
