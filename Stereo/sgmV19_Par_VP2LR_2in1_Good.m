...
AGCostSUMR=single(zeros(Rows,Cols,disparityRangeLevels));
 ...
 if SGM
     ...
     w=3;sig=[15,0.02]; %w=15;sig=[15,0.02]; Teddy good
     [AGCostSUML,AGCostSUMR]=SADBlock(left_in,right_in,w,disparityRange,w,sig,'aBienable');
 end
...
[LRRL,EffPixel]=LRRLConsistency(disparityMapSUML,disparityMapSUMR,2);
EffPixel*100

disparityMapSUMLM1=disparityMapSUMLM.*LRRL;

[rms,rmsall,perCorr,perCorrall]=RMSErr(disparityMapSUMLM,LRRL,2);
rms
rmsall
perCorr*100
perCorrall*100
...
figure
imagesc(255*disparityMapSUMLM/max(max(disparityMapSUMLM)))
colormap jet
colorbar
figure
imagesc(255*disparityMapSUMRM/max(max(disparityMapSUMRM)))
colormap jet
colorbar

if strcmp(ImageSource,'Own')
ThreeDReconstruction(disparityMapSUMLM,leftimg_col,0,cameracalib.calibrationSession.CameraParameters,LRRL)
end
...
left_in_3DRepMap=double(repmat(left_in,1,1,DisparityLevels)); % Create a 3D matrix from left_in_conv2, make it to be a moving window later
right_in_3DRepMap=double(repmat(right_in,1,1,DisparityLevels));
...
for d_window=1:DisparityLevels
    ...
    right_d_window(:,:,d_window)=[zeros(Rows,d_window_val(d_window)),right_in(:,1:end-d_window_val(d_window))];
    %Padding zeros to right image, to create a moving window on right image
    %[0 RightImage data]-> [0 0 0 0 0 0 0 RightImage data]
    left_d_window(:,:,d_window)=[left_in(:,d_window_val(d_window)+1:end),zeros(Rows,d_window_val(d_window))];
    %Padding zeros to left image,to create a moving window on left image
    %[0 LeftImage data]-> [0 0 0 0 0 0 0 LeftImage data]
    end
end

DisparitySpaceImageLeft1=abs(left_in_3DRepMap(:,:,:)-right_d_window(:,:,:));
DisparitySpaceImageRight1=abs(right_in_3DRepMap(:,:,:)-left_d_window(:,:,:)); %Calculate SAD

DisparitySpaceImageLeft=zeros(Rows,Cols,DisparityLevels);
DisparitySpaceImageRight=DisparitySpaceImageLeft;

Window=ones(WindowSize,WindowSize);
for i_level=1:DisparityLevels
DisparitySpaceImageLeft(:,:,i_level)=conv2(double(DisparitySpaceImageLeft1(:,:,i_level)),Window,'same'); %Get a value of sum of its neighbourhood at center
DisparitySpaceImageRight(:,:,i_level)=conv2(double(DisparitySpaceImageRight1(:,:,i_level)),Window,'same');
end
