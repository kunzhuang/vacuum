...
PixelCost=CalculatePixelCostRight(right_in,left_in,disparityRange);
...
%AGCost_Par=single(zeros(Rows,Cols,disparityRangeLevels,16));
AGCostSUM=single(zeros(Rows,Cols,disparityRangeLevels));
...
parfor p=1:PathNum
AGCostSUM=AGCostSUM+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,right_in);
end


[~,PixelCostDisp]=min(PixelCost,[],3);
figure
imshow(255*PixelCostDisp/max(max(PixelCostDisp)),[1,255])

[~,disparityMapSUM]=min(AGCostSUM,[],3);
disparityMapSUM=disparityMapSUM+64;
...
xyzPoints = reconstructScene(disparityMapSUMMed,CamCalib);
...
function PixCost=CalculatePixelCostRight(LeftImg,RightImg,disparityRange)

DisparityArrary=disparityRange(1):disparityRange(2);
LeftImgZPad=[zeros(size(LeftImg,1),1),LeftImg,zeros(size(LeftImg,1),disparityRange(2)+1)]; % Zero pad, left image + disparityRange(2)+1, right, +1
RightImgZPad=[zeros(size(LeftImg,1),1),RightImg,zeros(size(LeftImg,1),disparityRange(2)+1)];% Zero pad, right image + disparityRange(2)+1, right, +1
...
for Col=1:size(LeftImg,2)
    ...
    LeftValueOnCol(:,1,:)=repmat(LeftImgZPad(:,1+Col),[1,1,size(DisparityArrary,2)]); %Create a 3D matrix 1x1xsize(DisparityArrary,2), 1x1 from input image value at Col,Constant on Col
                                              %Due to zero padding, first
                                              %col statts from dRange+1+Col
                                              %on the left
    ...
    LeftValueOnColT3_Raw(:,:,:)=repmat(LeftImgZPad(:,1+Col-1:1+Col+1),[1,1,size(DisparityArrary,2)]); %Constant on Col, xl=xl, xr=xl-disparity
                                              %Find the corresponding 3
                                              %-1,col,+1
                                              %cols on left image as well
...
%%
function AGCost=PathScan(p,RR,CC,Corn,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,Img)
AGCost=single(zeros(Rows,Cols,disparityRangeLevels));
