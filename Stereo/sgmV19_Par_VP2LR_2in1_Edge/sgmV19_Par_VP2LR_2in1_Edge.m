...
switch ImageSource
    ...
    case 'MB'
        ...
        left_rectf_crop=rgb2gray(imread('con0.png'));right_rectf_crop=rgb2gray(imread('con1.png'));
end
...
%% SIFT match
if SIFT
%run('C:\Users\zk\Desktop\V3\vlfeat-0.9.20\toolbox\vl_setup')
...
AGCostSUMR=single(zeros(Rows,Cols,disparityRangeLevels));
 ...
 if SGM
     ...
     %SmallP=30;LargeP=800; %My data
      SmallP=5;LargeP=1*64; %Teddy Good
     ...
     parfor p=1:PathNum %Left image
         AGCostSUML=AGCostSUML+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCostL,left_in,left_rectf_crop_Canny,EdgeMode,0,1);
     end
     
     parfor p=1:PathNum %Right image
         AGCostSUMR=AGCostSUMR+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCostR,right_in,right_rectf_crop_Canny,EdgeMode,0,2);
     end
 else
...
function AGCost=PathScan(p,RR,CC,Corn,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,Img,EdgeCoord,CheckEdge,Save,s)
    ...
    for Col=1:Cols
        ...
        for Row=1:Rows
            ...
            if (LrLastRow <= 0 | LrLastRow >= Rows | LrLastCol<= 0| LrLastCol>= Cols) %check borders
                ...
                %CheckEdge;
                if CheckEdge==1
                    if EdgeCoord(Row,Col)
                        LrLastMinOthers=(LrLastMinOthers+SmallP)'; %Check if current pixle is on edge
                    else
    ...
    for Col=Cols:-1:1
        ...
        for Row=Rows:-1:1
            ...
            if (LrLastRow <= 0 | LrLastRow >= Rows | LrLastCol<= 0| LrLastCol>= Cols) %check borders
                ...
                if CheckEdge==1
                    if EdgeCoord(Row,Col)
                        LrLastMinOthers=(LrLastMinOthers+SmallP)';
                    else
                        LP=LargeP/double((abs(Img(LrLastRow,LrLastCol)-Img(Row,Col))));
                        if LP==Inf
                            LP=LargeP;
