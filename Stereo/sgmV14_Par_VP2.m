...
parfor p=1:PathNum
%AGCost_Par(:,:,:,p)=
AGCostSUM=AGCostSUM+PathScan(p,Path(p).row,Path(p).col,Path(p).cor,SmallP,LargeP,Cols,Rows,disparityRangeLevels,PixelCost,left_in);
%AGCostSUM=AGCostSUM+AGCost_Par(:,:,:,p);
%AGCost_Par(:,:,:,p)=0;
end
...
disparityMapSUM=disparityMapSUM;
